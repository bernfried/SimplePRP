...
/**
 * Handles requests for currencys.
 */
@Controller
...
public class CurrencyController {
     ...
     * @return Currency
     */
    @RequestMapping(value = "/{currencyId}", method = RequestMethod.GET, produces = { "application/json", "application/xml" })
    @ResponseBody
    public ResponseEntity<Currency> getCurrencyById(@PathVariable Long currencyId) {
        logger.info("CurrencyController - getCurrencyById: currencyId = '" + currencyId + "'");
    ...
    ResponseEntity<JaxbList<Currency>> getCurrencies() {
        ...
        JaxbList<Currency> jaxbList = new JaxbList<Currency>(currencys);
     ...
     * @throws Exception
     */
    @RequestMapping(value = "/{currencyId}", method = RequestMethod.DELETE, produces = { "application/json", "application/xml" })
    public ResponseEntity<Currency> deleteCurrencyById(@PathVariable Long currencyId) throws Exception {
        logger.info("CurrencyController - deleteCurrencyById: currencyId = '" + currencyId + "'");

        // check if id exists
        if (currencyId != null && !currencyService.exists(currencyId)) {
            throw new ObjectNotFoundException("Currency with id '" + currencyId + "' not found.");
        }
        ...
        if (!currencyService.exists(currencyId)) {
            throw new ObjectDeletionFailedException("Currency with id '" + currencyId + "' could not be deleted.", deletedCurrency);
        }
    ...
    public ResponseEntity<Currency> createCurrency(@RequestBody Currency currency, UriComponentsBuilder builder) throws Exception {
        ...
        if (existsCurrency != null) {
            throw new ObjectExistsAlreadyException("Currency with name '" + currency.getName() + "' exists already.", existsCurrency);
        }
        ...
        headers.setLocation(builder.path("/currencys/{id}").buildAndExpand(newCurrency.getId()).toUri());

        // return new currency
        return new ResponseEntity<Currency>(newCurrency, headers, HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<Currency> updateCurrency(@RequestBody Currency currency, UriComponentsBuilder builder) throws Exception {
        logger.info("CurrencyController - updateCurrency: reached");

        // check if a currency with that name exists
        Currency existsCurrency = currencyService.getByName(currency.getName());
        if (existsCurrency == null) {
            throw new ObjectNotFoundException("Currency with name '" + currency.getName() + "' not found.");
        }
        ...
        // set http header (location)
        HttpHeaders headers = new HttpHeaders();
        headers.setLocation(builder.path("/currencys/{id}").buildAndExpand(updatedCurrency.getId()).toUri());

        // return updated currency
        return new ResponseEntity<Currency>(updatedCurrency, headers, HttpStatus.OK);
    }
